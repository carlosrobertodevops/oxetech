...
#
# Esta e a variante **Colab** do projeto final. Monta o Google Drive e le os CSVs de uma
# pasta no seu MyDrive. Passos:
#
# 1. Faca upload dos 3 arquivos `NEAC_*_2022.csv` para uma pasta no Drive, ex.:
#    `MyDrive/oxetech/projeto-final/dados/`.
# 2. Ajuste a variavel `DADOS_DRIVE` abaixo para essa pasta.
# 3. Rode tudo (Ambiente de execucao > Executar tudo). Instalacao de dependencias e montagem
#    do Drive acontecem automaticamente.
#
# Fora do Colab (Jupyter/local ou REPL do Zed), a deteccao cai automaticamente para `./dados`.

# %%
# Instala dependencias que podem faltar no Colab (a maioria ja vem pre-instalada).
import sys, subprocess
_pacotes = ["shap", "fastapi", "uvicorn", "joblib", "jupytext"]
try:
    subprocess.run([sys.executable, "-m", "pip", "install", "-q", *_pacotes], check=False)
except Exception as _e:
    print("Aviso: falha ao instalar pacotes opcionais:", _e)

# %%
# Caminho dos CSVs no Google Drive. AJUSTE para a sua pasta.
# Deixe como esta se usou MyDrive/oxetech/projeto-final/dados/.
DADOS_DRIVE = "/content/drive/MyDrive/oxetech/projeto-final/dados"
